...
void BulletManager::LaunchPlayerBullet(float x, float y, float sr, float ssx, float ssy, bool isHoming) {
	for (int i = 0; i < PlayerBMax; i++) {
		if (!p_bullets[i].GetFlag()) {
			p_bullets[i].Shoot(x, y, sr, ssx, ssy, isHoming, Bullet::OwnerType::PLAYER);
			break;
		}
	}
}
	...
	for (int i = 0; i < PlayerBMax; i++) {
		...
		if (p_bullets[i].isHoming) {
			if (p_bullets[i].owner == Bullet::OwnerType::PLAYER) {			//OwnerTypeはBulletクラス内で定義しているので、BulletクラスのOwnerTypeであることを明示する。
				if (bossAlive) {
					p_bullets[i].Guide(bossX, bossY, 0.1f);						//OwnerTypeでPLAYERがなので自機が出す弾はボスを狙うからボス座標を引数とする。
				}
